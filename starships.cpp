...
StarShips::StarShips(sf::RenderWindow* window) {
    runCount = 0;
    dtRenderer.window = window;
    dtRenderer.matrix = &matrix;
}

void StarShips::initialize(int initMethod) {
    if (initMethod == 0) {
...
void StarShips::render() {
    dtRenderer.renderMatrix();
}

int StarShips::shouldLive(int posX, int posY) {
    int neighbors[8];
    ...
    if (!(posX % 30 == 0) && !(((posY + runCount) % 30) % 10 == 0)) {
        return currentState;
    }
    ...
    if (currentState > 0) {
        ...
        // overflow
        if (currentState > 0xFFFFFF * 2) {
            alive = 0;
        }
    } else if (currentState == 0) {
        // reproduction
        if (livingNeighbors > 300000 && livingNeighbors < 9000000) {
